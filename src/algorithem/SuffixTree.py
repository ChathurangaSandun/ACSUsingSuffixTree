...
def longestSubstring(A,B):
        ...
        for i in range(0,n):
            ...
            while (suffixTree.find_substring(substring) != -1) :
                length += 1
                j+= 1
...
def similarity(longestSubstring,m):
    return longestSubstring/math.log10(m)

def distance (longestSubstringAB,n,longestSubstringAA,m):
    return (math.log10(m)/longestSubstringAB)-(math.log10(n)/longestSubstringAA)
...
def calculateACS(A,B):
    
    
    A = str(inputA.read()).strip()
    B = str(inputB.read()).strip()
    ...
    longestSubstringAB = longestSubstring(A,B)
    #print(longestSubstringAB)
    longestSubstringBA = longestSubstring(B,A)
    #print(longestSubstringBA)
    longestSubstringAA = n/2
    #print(longestSubstringAA)    
    longestSubstringBB = m/2
    #print(longestSubstringBB)
    
    
    distanceAB = distance(longestSubstringAB, n, longestSubstringAA, m)
    #print(distanceAB)
    distanceBA = distance(longestSubstringBA, m, longestSubstringBB, n)
    #print(distanceBA)
    
    acs = ACS(distanceAB, distanceBA)
    
    acs = float("{0:.5f}".format(acs))
    
    return acs

'''
inputA = open("Data set/1 V00654.txt",'r')
inputB = open("Data set/2 X61145.txt",'r')

a = calculateACS(inputA, inputB)
...
if __name__ == '__main__':
    ...
    files = ["Data set/1 V00654.txt",
             "Data set/2 X61145.txt",
             "Data set/3 X72204.txt",
             "Data set/4 X63726.txt",
             "Data set/5 X72004.txt",
             "Data set/6 U20653.txt",
             "Data set/7 X79547.txt",
             "Data set/8 X97336.txt",
             "Data set/9 V00711.txt",
             "Data set/10 X14848.txt",
             "Data set/11 D38112.txt",
             "Data set/12 D38113.txt",
             "Data set/13 D38116.txt",
             "Data set/14 D38114.txt",
             "Data set/15 D38115.txt",
             "Data set/16 X97707.txt",
             "Data set/17 X99256.txt",
             "Data set/18 Z29573.txt",
             "Data set/19 Y10524.txt",
             "Data set/20 X83427.txt"
             ]
    ...
    for i in range(0,len(files)):
        for j in range(i+1,len(files)):
            ...
            text_file.write(files[i] +"        "+files[j]+"\n")
            print(files[i] +"        "+files[j])
            
            inputA = open(files[i],'r')
            inputB = open(files[j],'r')
            
            acs = calculateACS(inputA, inputB)
            matrix[i][j] = acs
            matrix[j][i] = acs
            
            print("ACS - "+str(acs))
            text_file.write("ACS - "+str(acs)+"\n")
            
            runingTime = time.time() - start_time
            print("--- %s seconds ---" % str(runingTime))
            text_file.write("--- %s seconds ---" % runingTime+"\n")
            print()
            text_file.write("\n")  
    ...
    print("Average Common Substring (ACS) Algorithm - Chathuranga Sandun/Shalika Fernando ")    
